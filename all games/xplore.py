...
thrustgrow = pygame.image.load("tankgame\\img\\thrustgrow.png").convert_alpha()
thrustflickr = pygame.image.load("tankgame\\img\\thrustflickr.png").convert_alpha()
thurst = [thrustgrow,thrustflickr]
global thx,thy,thn
thx = 0
thy = 0
thn = 0
...
class astroidgen:
    ...
    def __init__(self,startpx,startpy,img):
        self.startpx = startpx
        self.startpy = startpy
        self.img = img
        self.directionx = random.randint(-3,3)
        self.directiony = random.randint(-3,3)        

    def move(self):
        self.startpx += self.directionx*random.randint(0,5)
        self.startpy += self.directiony*random.randint(0,5)
        screen.blit(astroidsimg[self.img],(x+self.startpx,y+self.startpy))
...
def level1():
    ...
    while not level1:
        ...
        distance = math.sqrt((mousey-y)*(mousey-y)+(mousex-x)*(mousex-x))
        
        angle_radians = math.atan2((mousey-y),(mousex-x))
        angle = 360 - math.degrees(angle_radians)

        if distance != 0:
            speed = speed + inc
            xchange = speed*(mousex-x)/distance
            ychange = speed*(mousey-y)/distance
        if distance >=100:
            x = x+xchange
            y = y+ychange
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                bullets.append(bullet(color,x+50*math.cos(angle_radians),y+50*math.sin(angle_radians),angle_radians))
                pass
        ...
        rect = palletterot.get_rect(center = (x,y))
        screen.blit(palletterot,rect)

        rect = tankbaserot.get_rect(center = (x,y))
...
def main():
    ...
    while not game_exit:
        ...
        if random.randint(0,2) is 1:
            astroids.append(astroidgen(random.randint(0,12000-screen_width),random.randint(0,7000-screen_height),random.randint(0,8)))
